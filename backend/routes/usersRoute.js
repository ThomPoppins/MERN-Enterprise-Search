...
router.get('/search/:searchTerm', async (request, response) => {
  try {
    ...
    if (company !== null && Array.isArray(company.owners)) {
...
router.get('/user/:id', async (request, response) => {
  try {
    ...
    if (userDocument === null) {
      console.log('userDocument === NULL, User not found in usersRoute.js.')
    ...
    if (user.profilePicture) {
      ...
      Promise.resolve(image).then((image) => {
        if (!image) {
          return
        }

        if (!user.profilePictureURL) {
          // Get the URL to the profile picture file
          profilePictureURL = getStaticFileURLFromPath(image.path)

          // Save the updated user to the database
          user
            // @ts-ignore
            .updateOne({ profilePictureURL })
            .then((result) => {
              console.log('Result saving user call: ', result)
            })
            .catch((error) => {
    ...
    // Count how many invites the user has with status "pending"
